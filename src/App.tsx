...
import { Zap, Upload, Download, RotateCcw, MessageSquare, TerminalSquare, Loader2, Expand, Shrink, Image as ImageIcon, X, AlignLeft, AlignCenter, AlignRight, Type, ArrowDownUp, Volume2, VolumeX, Play, Square } from 'lucide-react';
...
export default function App() {
  ...
  }, [output, ttsIsPlaying, ttsStop, ttsSpeak]);
      ...
      if (provider === 'GEMINI') {
        ...
        for (const model of candidateModels) {
          ...
          try {
            ...
            if (!response.ok) {
              throw new Error(`Proxy not available status ${response.status}`);
            }
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-transparent relative z-0 h-full">
           ...
           <div className="h-10 border-b border-[var(--border-color)] flex items-stretch justify-between px-2 sm:px-4 shrink-0 bg-[var(--bg-color-panel)] transition-colors duration-300 overflow-x-auto relative z-10">
              ...
              <div className="flex items-center gap-3 shrink-0 h-full text-[var(--text-color-dim)] pr-2 hidden md:flex">
                 ...
                 {/* TTS Quick Play Button */}
                 <button 
                   onClick={ttsPlayFromTop}
                   disabled={!output}
                   className={`flex items-center gap-1 px-3 py-1 rounded-sm border text-[11px] font-bold transition-all disabled:opacity-30 disabled:cursor-not-allowed ${
                     ttsIsPlaying 
                       ? 'bg-[var(--accent-color)] text-black border-[var(--accent-color)] animate-pulse' 
                       : 'bg-[var(--bg-color-base)] border-[var(--border-color)] text-[var(--text-color-dim)] hover:bg-[var(--border-color)] hover:text-[var(--active-text-color,var(--text-color-highlight))]'
                   }`}
                   title={language === 'EN' ? 'Read Aloud (TTS)' : '音声読み上げ (PLAY/STOP)'}
                 >
                   {ttsIsPlaying ? <VolumeX size={12} /> : <Volume2 size={12} />}
                   <span>{ttsIsPlaying ? (language === 'EN' ? 'STOP' : '停止') : (language === 'EN' ? 'PLAY' : '再生')}</span>
                 </button>
              ...
              <div className={`absolute bottom-6 right-6 flex flex-col gap-2 z-20 transition-transform duration-300`}>
                 ...
                 {activeTab === 'SEARCH_BUFFER' && output && (
                   ...
                   <>
                      ...
                      <button 
                        onClick={ttsPlayFromTop} 
                        title={ttsIsPlaying ? (language === "EN" ? "Stop Voice" : "音声停止") : (language === "EN" ? "Read Aloud" : "音声読み上げ (PLAY)")} 
                        className={`w-10 h-10 border rounded-sm flex items-center justify-center transition-all shadow-lg ${
                          ttsIsPlaying 
                            ? 'bg-[var(--accent-color)] text-black border-[var(--accent-color)] animate-pulse' 
                            : 'bg-[var(--bg-color-base)] border-[var(--border-color)] text-[var(--text-color-dim)] hover:bg-[var(--border-color)] hover:text-[var(--active-text-color,var(--text-color-highlight))]'
                        }`}
                      >
                        {ttsIsPlaying ? <VolumeX size={16} /> : <Volume2 size={16} />}
                      </button>
